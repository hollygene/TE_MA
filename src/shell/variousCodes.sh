...
awk 'NR==FNR{a[$1,$2]; next} !(($1,$2) in a)' snpsAllD1.txt D1_noLow_noHigh_redGem_AncCalls_NoHetsVars.vcf > notShared_filtered.txt




awk 'NR==FNR{a[$1,$2]; next} (($2,$3) in a)' H0_noLow_noHigh_redGem_AncCalls_NoHetsVars.vcf H0_vcf_all_NEWCALLSv2.txt > H0_vcf_Dave_Calls_Filtered.txt
awk 'NR==FNR{a[$1,$2]; next} !(($2,$3) in a)' H0_noLow_noHigh_redGem_AncCalls_NoHetsVars.vcf H0_vcf_all_NEWCALLSv2.txt > filtered_out_H0.txt 
...
###rename multiple fiels (remove part of name)
