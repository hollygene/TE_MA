...
h0_triNT <- read.table("/Users/hollymcqueary/Dropbox/McQueary/Paradoxus_MA/VCFs/H0/H0_trinucleotideContext.txt",header=TRUE)
h0_0_1 <- subset(h0_triNT, context=="GCG")
h0_0_2 <- subset(h0_triNT, context=="CAG")

h0_triNT_plot <- qplot(x=context,y=Relative_Mutation_Rate, data=h0_triNT, geom="point")
h0_triNT_plot + theme_classic() + theme(axis.text.x = element_text(angle = 90)) + labs(y="Relative Mutation Rate",x="") + scale_y_continuous(n.breaks=13, limits=c(0,14)) + geom_point(data=h0_0_1, colour="red") + geom_point(data=h0_0_2, colour="red")
last_plot()
