...
```{r karyoploteR, include=TRUE, echo=FALSE}
# if (!requireNamespace("BiocManager", quietly = TRUE))
#     install.packages("BiocManager")
# 
...
TEs_1 <- read.table("/Users/hollymcqueary/Dropbox/McQueary/Paradoxus_MA/BedGraphs/TELocate/D20/raw/HM-D20-1_R1_001_val_telocate_raw.TY1.bed")
...
{pdf("kpPlotD20-10.pdf")
  kp10 <- plotKaryotype(genome=Chr_sizes_GR,cytobands=gene_bed_GR,chromosomes= chroms, cex=0.5)
# this tells the program to add base numbers (self-explanatory)
kpAddBaseNumbers(kp10,tick.dist = 100000,add.units = TRUE)
# this plots whatever markers you tell it to - in this case, markers of TEs in this genome
kpPlotMarkers(kp10, data=TEs_10, chr=TEs_10$chr, x=TEs_10$start, labels=TEs_10$TE, data.panel=2,cex=0.5, text.orientation = "horizontal", line.color = "pink", label.color = "purple", label.dist = 0.01, max.iter = 1000)
...
# this plots the coverage in 10kb chunks bc that's what we computed
kpBars(kp10, data=D20_10_GR, y1=D20_10_GR$coverage, ymax=max(D20_10_GR$coverage)/2, data.panel=1)
dev.off()}
#, r0=0, r1=0.25)}
# kpBars(kp1, data=D20_A_GR, y1=D20_A_GR$coverage, ymax=max(D20_A_GR$coverage)/2, data.panel = 1, col="blue",r0=0.25, r1=0.5)
